...
def aggregate_pair_scores(pair_scores_csv: Path, output_csv: Path) -> pd.DataFrame:
    pair_scores = pd.read_csv(pair_scores_csv)
        ...
        rows.append(
            {
                ...
                "S_aff": noisy_or(group["s_aff_pair"].to_numpy()),
...
def main() -> None:
    ...
    if args.command == "aggregate":
        df = aggregate_pair_scores(Path(args.pair_scores), Path(args.output))
